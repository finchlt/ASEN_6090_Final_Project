function [ fid, rec_xyz, data, rinexv3] = read_rinex_obs(fname, PRN_list, maxlines)
%*******************************************************
...
for i = 1:length(observables)
	data.(cell2mat(observables(i))) = zeros(32,1);
end

% Status
%disp([ 'Parsing RINEX file ' fname ]);
if nargin >= 2 && ~isempty(PRN_list)
    fprintf('\n\t=> Returning data for PRNs: %s\n', num2str(PRN_list));
else
...
while (current_line ~= -1) & (line_count < maxlines)
    ...
    GPSTime(1,epoch) = gpswk;
	 GPSTime(2,epoch) = gpssec;
    ...
    for ii=1:current_num_sv 
	 ...
	 %grow the data struct to accomodate another epoch
	 for i = 1:length(observables)
		 data.(cell2mat(observables(i)))(1,epoch) = 0;
	 end
...
% 		 end
        ...
        if (sat_type(ii) ~= 'G') & (sat_type(ii) ~= ' ')
		  ...
		  for i = 1:length(observables)
			  data.(cell2mat(observables(i)))(current_prn(ii),epoch) = current_obs(i);
		  end
...
data.('GPSSec') = GPSTime(2,:);
data.('GPSWeek') = GPSTime(1,:);
